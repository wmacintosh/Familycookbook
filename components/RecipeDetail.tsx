...
const RecipeDetail: React.FC<RecipeDetailProps> = ({ recipe, onBack, isFavorite, onToggleFavorite, onEdit, onDelete, onUpdateRecipe }) => {
  ...
  const getGeminiTips = async () => {
    ...
    setErrorTips(null);
    try {
      const tips = await apiClient.getTips(recipe.title, recipe.ingredients);
      setTips(tips);
    } catch (error: any) {
      console.error(error);
      setErrorTips(error.message || "AI service unavailable.");
    } finally {
      setLoadingTips(false);
    }
  };
  ...
  const searchSubstitutions = async () => {
    ...
    setSearchResults(null);
    try {
      const result = await apiClient.searchSubstitutions(recipe.title, searchQuery);
      setSearchResults(result);
    } catch (error: any) {
      console.error(error);
      setSearchResults({ text: "Failed to search. Please try again.", links: [] });
    } finally {
  ...
  const generateImage = async () => {
    ...
    setImageError(null);
    try {
      const imageUrl = await apiClient.generateImage(recipe.title, recipe.description, imageSize);
      onUpdateRecipe({ ...recipe, imageUrl });
    } catch (error: any) {
      console.error(error);
  ...
  const readRecipe = async () => {
    ...
    setIsReading(true);
    try {
      const base64Audio = await apiClient.generateAudio(recipe.title, recipe.ingredients, recipe.instructions);
